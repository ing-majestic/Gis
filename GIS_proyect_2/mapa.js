    ...
    var heat_l = L.markerClusterGroup();

    fetch('http://localhost:8080/geoserver/'+workspace+'/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=Curso_GIS%3A'+nombre_capa+'&maxFeatures='+max_features+'&outputFormat='+output_format)
    ...
    features.forEach(elemento => {
        ...
        points_heat.push([latitud,longitud]);
         ...
         //heatmap
    var heat = L.heatLayer(points_heat, {radius: 20});
    ...
    var map = L.map('map', {
		center: [19.393060, -99.138743],
		zoom: 11,
		layers: [dark, cities, markers, heat_l]
	});
